            ...
            <div className="space-y-2">
              {tasks.map((task) => (
                <div key={task.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <span className={`text-sm ${task.status === "concluida" ? "line-through text-gray-400" : "text-gray-700"}`}>
                    {task.title}
                  </span>
                  <span className={`text-xs px-2 py-1 rounded-full ${
                    task.priority === "alta" ? "bg-red-100 text-red-700" :
                    task.priority === "media" ? "bg-yellow-100 text-yellow-700" : "bg-gray-200 text-gray-600"
                  }`}>
              ...
              {invoices.map((inv) => (
                <div key={inv.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${
                    ...
                    {inv.status === "pago" ? "Pago" : inv.status === "atrasado" ? "Atrasado" : "Pendente"}
                  </span>
                </div>
              ))}
            </div>
